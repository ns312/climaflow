...
def get_ai_response(chat_id, user_message):
    ...
    if has_lead:
        # Проверяем, не является ли это новым запросом на заказ/услугу
        keywords = ["еще", "новый", "другой", "заказать", "заказ", "чистк", "ремонт", "установк", "заправк", "брат", "мам", "колонн"]
        is_new_request = any(k in user_message.lower() for k in keywords)
